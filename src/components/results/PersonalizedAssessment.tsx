...
const PersonalizedAssessment: React.FC<PersonalizedAssessmentProps> = ({ 
  ...
  const content = getDynamicContent(scoreCategory, capitalizedFirstName, primarySymptoms);
    ...
    <section className="reveal-section transform opacity-0 py-8 px-4 md:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        <div className="bg-gradient-to-br from-[#f8f5ff] to-white rounded-xl shadow-lg border-2 border-white p-8 md:p-10">
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-headline font-bold text-primary mb-8 leading-tight">
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white/60 rounded-lg p-6 border border-purple-100">
              <p className="text-lg md:text-xl text-[#7E69AB] font-medium leading-relaxed">
                {content.validation}
              </p>
            </div>
            
            <div className="bg-white/60 rounded-lg p-6 border border-purple-100">
              <p className="text-lg md:text-xl text-[#6E59A5] leading-relaxed">
                {content.explanation}
              </p>
            </div>
              ...
              <div className="space-y-4">
                {primarySymptoms.map((symptom, index) => (
                  <div key={index} className="flex items-start">
                    ...
                    <span className="text-xl md:text-2xl text-warm-gray font-medium leading-relaxed">
                      {symptom}
                    </span>
                  </div>
                ))}
