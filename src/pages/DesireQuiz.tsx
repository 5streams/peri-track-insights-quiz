      ...
      <main className="max-w-2xl mx-auto px-5 pt-6 pb-16">
        {screen.step === WELCOME_STEP && (
          <div className="text-center pt-8">
            <div className="text-[11px] tracking-[0.18em] uppercase text-[#8B5A9F] font-bold mb-4">
              A private 3-minute assessment
            </div>
            <h1 className="font-playfair text-[32px] md:text-4xl font-semibold text-[#331D2E] leading-tight mb-4">
              Why did the wanting disappear?
            </h1>
            <p className="text-[17px] leading-relaxed text-[#5c4553] mb-6 max-w-lg mx-auto">
              You love him. The desire just… left. This 3-minute assessment shows you what's actually happening — and whether it can come back.
            </p>
            <button
              onClick={() => goTo({ step: AGE_STEP })}
              className="inline-block w-full max-w-sm px-8 py-4 rounded-full font-bold text-white text-[17px] transition"
              style={{ background: "linear-gradient(135deg,#A4688F 0%,#46293F 100%)", boxShadow: "0 10px 30px rgba(70,41,63,.22)" }}
            >
              Start
            </button>
            <p className="text-xs text-[#6E5665] mt-5">22 questions · Private · Nothing is shared</p>
          </div>
        )}
