...
interface PDFMergerLeadModalProps {
  isOpen: boolean;
  onClose: () => void;
}

const PDFMergerLeadModal: React.FC<PDFMergerLeadModalProps> = ({
  isOpen,
  onClose
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const additionalNotes = `PDF Merger trial signup from /pdfmerger page at ${new Date().toISOString()}`;
      
      const trialQuizResults = {
        source: "PDF_MERGER_TRIAL_SIGNUP",
        phase: "PDF Merger Trial Interest",
        score: 100,
      ...
      console.log("PDFMergerLeadModal: About to save lead with data:", {
        name: name.trim(),
        email: email.trim(),
        source: 'TRIAL',
        pricingTier: 'PDFMergerTrial',
          ...
          <>
            <DialogHeader>
              ...
              <p className="text-gray-600 mt-2">
                Get 14 days of unlimited PDF merging and document processing.
              </p>
              <p className="text-blue-600 font-medium text-sm mt-1">
                Then only $12/month • Cancel anytime
              </p>
            </DialogHeader>
...
export default PDFMergerLeadModal;
