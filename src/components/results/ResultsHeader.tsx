...
const ResultsHeader: React.FC<ResultsHeaderProps> = ({ 
  ...
  useEffect(() => {
    // Immediately set the score fill width without animation
    if (scoreRef.current) {
    ...
    <header className="text-center pt-6 pb-12">
      ...
      <div className="relative max-w-2xl mx-auto mb-10">
        <div className="mb-3 flex items-center justify-center">
          <h2 className="text-xl md:text-2xl font-medium text-[#5D4154] inline-block relative">
            ...
            <Badge 
              variant="outline" 
              className={`ml-3 px-4 py-1.5 text-lg md:text-xl ${getScoreColor()} border-2`}
            >
              {score}/100
            </Badge>
          </h2>
        ...
        <div className="relative h-12 bg-gray-100 border border-gray-200 rounded-full w-full mb-7 shadow-inner overflow-hidden">
          <div 
            ref={scoreRef}
            className={`absolute left-0 top-0 h-full rounded-full ${getMeterFillColor()}`}
            style={{ width: "0%" }} // Initial width, will be set in useEffect
          />
          
          <div className="absolute w-full top-0 h-full flex items-center">
      ...
      {scoreCategory === "severe" && (
        <Button 
          ...
          className="bg-gradient-to-r from-[#9b87f5] to-[#8B5CF6] hover:from-[#8B5CF6]/95 hover:to-[#8B5CF6] text-white font-semibold px-8 py-6 rounded-lg shadow-lg mb-8 text-lg transition-all duration-300"
        >
